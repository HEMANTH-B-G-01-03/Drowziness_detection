...
model_points = np.array([
    (0.0, 0.0, 0.0),  # Reference point – tip of the nose
    (-30.0, -125.0, -30.0),  # Left corner of the eye
    (30.0, -125.0, -30.0),  # Right corner of the eye
    (-60.0, -70.0, -60.0),  # Left corner of the mouth
    (60.0, -70.0, -60.0),  # right corner of the mouth
    (0.0, -330.0, -65.0)    # chin
])
...
weights_path = os.path.abspath("weights/yolov5m.pt")  # Update with the exact path to the weights file
...
while True:
    ...
    if len(faces)==0 : 
        # cv2.putText(img, ""Driver not looking ahead"", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
         sound_thread("look")

    results = model(img)
    detections = results[0].boxes.data
    for detection in detections:
        if int(detection[5]) == 67:  # 67 is the index of 'cell phone'
    ...
    for face in faces:
            ...
            #    Extract specific image points for solvePnP
                image_points = np.array([
                    landmarks_points[30],  # Tip of the node 
                    landmarks_points[36],  # left corner of eye
                    landmarks_points[45],  # right cornerof the eye 
                    landmarks_points[48],  # left corner of mouth
                    landmarks_points[54],  # right corner of the mouth
...
#integration for the backend part of the project is bellow 
...
#     head_tilt_degree = abs(
#         [-180] - np.rad2deg([
#             rotationMatrixToEulerAngles(rotation_matrix)[0]
#         ])
#     )
...
#     vertical_distance = dist.euclidean(
#         nose[0],
#         nose[2]
#     )

#     depth_distance = dist.euclidean(
#         nose[0],
#         nose[1]
#     )
...
#         vector_horizontal /
#         np.linalg.norm(vector_horizontal)
#     )
